...
> 高级轮式载具是编辑器提供的一种逻辑功能对象，用于快速实现一个具有模拟物理特性的轮式地面载具
> 高级轮式载具可以通过模型或特效搭建载具外观。
...
在[资源库]中选择[游戏功能对象]，搜索[高级轮式载具]，找到功能对象将其拖拽到场景中，创建一个高级轮式载具逻辑功能对象。
...
| 摩擦力系数    | 设置载具的轮胎摩擦力，摩擦力影响载具的运行效果，摩擦力为0时，轮胎与地面摩擦力失效，会导致载具无法移动。| number| 0 - 8|
| 发动机转速(RPM) | 设置每分钟发动机的最大转速，转速越大，载具可达到的最大速度更大。(注意，此处为载具发动机转速，并不代表实际运行时的KM/H速度)| number| 0 - 1000000|
| 加速系数  | 设置载具加速系数,加速效果基于物理模拟运算，直接修改加速系数并不会出现很大的效果差距，配合载具摩擦力、质量等其他参数可以进一步调整载具行驶速度| number| 0.1 - 100(N*m)|
...
| **动力轮组属性** | **说明**                                | **类型** | **取值范围** |
| --------------- | ---------------------------------------- | ------- | ------------ | 
| 轮胎半径         | 设置物理轮胎的半径                        | number | 0 - 1000(cm) |
...
- **step.2** **完成创建高级轮式载具对象创建后，可以进行载具外观的拼装**
从[资源库]中选择合适的模型或特效，放置在高级轮式载具对象子级，即完成车身外观的拼装。
用于拼装的模型会自动成为载具碰撞的一部分。
...
- **step.3** **绑定轮胎**
可以绑定0~4个车轮，注意调整每个车轮在合适的位置，否则车轮位置会影响载具移动时的平衡性。
通过高级轮式载具属性面板中的轮胎吸管工具，选中场景中需要做为轮胎的模型或特效，即可完成绑定。
通过高级轮式载具属性面板中的轮胎半径，将轮胎碰撞碰撞盒调整到适合模型的大小。
高级轮式载具可以指定某个模型或特效做为载具的轮胎，绑定后模型或特效会具备轮胎转动的动画效果。

<video controls src="https://cdn.233xyx.com/athena/online/2180928441d0404894fe0564ceeb2ad9.mp4"></video>

::: tip 轮胎碰撞提示
高级轮式载具的轮胎碰撞只识别载具自带的碰撞盒，即载具中绿色的轮胎提示框，被绑定的轮胎模型不具有实际碰撞效果。
:::

## 如何测试载具运行效果

- **step.4** **调整触发器和交互物的位置**
完成载具的外观拼装后，可以根据外观调整载具默认的触发器和交互物位置。
触发器用于激活自动上车逻辑。
交互物用于绑定角色与载具关联的位置，设定角色的动画姿态。
