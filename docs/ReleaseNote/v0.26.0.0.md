...
<video controls src="https://arkimg.ark.online/026V1.mp4"></video>
...
- 梳理内部接口以更友好直观的形式提供，引导用户该对象具备的权限概念而不是简单的空气墙。
...
| **<color=#ff0000ff>****或****<color=#red>** | 根据参数值#rrggbbaa设置文本的颜色，分别表示颜色的红、绿、蓝和 Alpha（透明度）值，大小写都能识别20种常用颜色可以直接使用颜色名称，大小写都能识别 | We are <color=#ff0000ff>colorfully</color> amusedWe are <color=#red>colorfully</color> amused |
...
![img](https://meta.feishu.cn/space/api/box/stream/download/asynccode/?code=OGJkMzFhZmMyYmRjYjVkZjgwZDYxYTBhNjI3YmUwNTFfSkp4YlcxNTQxUHB1bHduS1owYnF3S0tPUGx5VzlNZW1fVG9rZW46TEd5aWJQU0tibzRhRWF4cGZhYWM2VFlEbm54XzE2OTIwMTM4MDk6MTY5MjAxNzQwOV9WNA)
...
- 碰撞属性从gameObject中迁移出来了，能修改碰撞属性的只有角色和模型。其他类型对象属性面板中不显示碰撞属性。026会兼容025，对于不再显示碰撞属性的对象（例如预制体根节点），依旧支持通过代码设置碰撞状态。
- 碰撞属性中去除了“从父对象获取”。之后将不再支持从父对象获取。
  - 如果想要一个对象下的部分子物体跟随父节点碰撞，另一部分不跟随碰撞，保持开启/关闭状态，需要单独处理这两部分内容。
  - 在025中设置为“从父对象获取”的对象，026中会自动转换成其真实碰撞状态。例如父节点为关闭碰撞，那么在026上子节点为关闭碰撞。如果父节点为开启碰撞，那么026上子节点为开启碰撞。
  - 如果一个预制体内部根节点为开启碰撞，子物体为“从父对象获取”，拖到场景中后将根节点改为“关闭碰撞”，这种情况在025会有bug，场景中的子物体碰撞状态会跟随预制体内部的真实碰撞状态。
