...
![024-1](E:\Typora图片放置目录\024-1.PNG)

- 五个组件名都去掉MW

![024-2](E:\Typora图片放置目录\024-2.PNG)
...
![024-3](E:\Typora图片放置目录\024-3.PNG)

- 使用此功能后，对象管理器生成组合头结点
  - 可以多层嵌套组合
  - ![024-4](E:\Typora图片放置目录\024-4.PNG)
- 主视口选中组合或者在对象管理器中选择组合头结点，右键可取消组合，快捷键：Shift+G

![024-5](E:\Typora图片放置目录\024-5.PNG)
...
![024-6](E:\Typora图片放置目录\024-6.PNG)

- 点击收藏后，在收藏按钮下方弹出弹窗
  - 文件夹通过下拉形式选择，默认选择默认文件夹
  - ![024-7](E:\Typora图片放置目录\024-7.png)

  - 点击下拉中的“自定义收藏夹”按钮，出现新增文件夹弹窗
    - ![024-8](E:\Typora图片放置目录\024-8.png)

    - ![024-9](E:\Typora图片放置目录\024-9.png)

    - 若出现新增重名文件夹，提示“名称重复”
    - 文件夹名称最多20个字符
    - ![024-10](E:\Typora图片放置目录\024-10.png)

    - 新增文件夹后同步至本地资源库——收藏目录
  - 生效条件：点击收藏按钮时，收藏生效，按钮变为已收藏状态
    - 所选文件夹为上次收藏时选择的文件夹（信息存在本地），首次收藏时选择默认文件夹
    - ![024-11](E:\Typora图片放置目录\024-11.png)
  - 点击弹窗之外的区域，弹窗关闭
  - 点击完成后，弹窗关闭
...
- 详情请见产品文档：[特效]([音效 | 产品手册 (ark.online)](https://docs.ark.online/GameplayObjects/Effects.html)) 
...
| <img src="E:\Typora图片放置目录\024-12.jpg" alt="024-12" style="zoom: 80%;" /> | ![024-13](E:\Typora图片放置目录\024-13.jpg) |
| ------------------------------------------------------------ | ------------------------------------------- |
|                                                              |                                             |
...
<video src="E:\Documents\Desktop\教程图片\rn图床\024\024-14弹力与摩擦力效果.mp4"></video>
...
![024-15](E:\Typora图片放置目录\024-15.png)

- 本次还更新了新建项目的默认UI的图片资源等参数，现在可以通过tag在脚本中找到默认UI对象了

| <img src="E:\Typora图片放置目录\024-16.png" alt="024-16" style="zoom:50%;" /> | ![024-17](E:\Typora图片放置目录\024-17.png) |
| ------------------------------------------------------------ | ------------------------------------------- |
|                                                              |                                             |
...
- 支持TS调用框选接口，实现对象多选
  ...
  - <video src="E:\Documents\Desktop\教程图片\rn图床\024\024-18框选示例.mp4"></video>
- 对TS开放高亮接口，并对多个对象高亮时的前后层级的显示关系单独处理，确保被遮挡的高亮对象能够正确显示
  
  - <video src="E:\Documents\Desktop\教程图片\rn图床\024\024-19高亮示例.mp4"></video>

Api详情

| **函数名称**                | **用法**                                                     | **说明**               | **参数**                                                     | **返回类型**               | **使用域** |
| --------------------------- | ------------------------------------------------------------ | ---------------------- | ------------------------------------------------------------ | -------------------------- | ---------- |
| getGameObjectBySelectionBox | Gameplay.getGameObjectBySelectionBox(this.touchPos1, this.touchPos2, false) | 获取框选屏幕位置的物体 | StartPoint(Type.Vector2)： 鼠标开始位置EndPoint(Type.Vector2)： 鼠标结束位置IsIncludeNonCollidingObjects(boolean)：是否包含物体非碰撞组件IsUseObjectsBoundingBox(boolean)：是否使用物体包围盒 | Array<HitResult>框选的物体 | Client     |
| drawGameObjectSelectionBox  | Gameplay.drawGameObjectSelectionBox(this.touchPos1, this.touchPos2, new Type.LinearColor(1, 1, 1, 0.5)); | 绘制物体选择框         | StartPoint(Type.Vector2)： 鼠标开始位置EndPoint(Type.Vector2)： 鼠标结束位置Color(Type.LinearColor)：选择框颜色DurationTime(number)：显示时间 | void                       | Client     |
| setGlobalOutlineParams      | Gameplay.setGlobalOutlineParams(4, 1, 0, 0, 1);              | 设置全局描边参数       | Width(number)：描边宽度CoveredAlpha(number)：被遮挡描边透明度CoveredEdgeAlpha(number)：被遮挡非描边透明度NotCoveredAlpha(number)：非遮挡描边透明度NotCoveredEdgeAlpha(number)：非遮挡非描边透明度 | void                       | Client     |

- 设置全局描边参数中不同值的效果

  - CoveredAlpha 被遮挡非描边部分透明度

  - | ![024-20](E:\Typora图片放置目录\024-20.jpg) | ![024-21](E:\Typora图片放置目录\024-21.jpg) |
    | ------------------------------------------- | ------------------------------------------- |
    | CoveredAlpha = 0                            | CoveredAlpha = 1                            |

  - CoveredEdgeAlpha 被遮挡描边透明度

  - | ![024-22](E:\Typora图片放置目录\024-22.jpg) | ![024-23](E:\Typora图片放置目录\024-23.jpg) |
    | ------------------------------------------- | ------------------------------------------- |
    | CoveredEdgeAlpha = 0                        | CoveredEdgeAlpha = 1                        |

  - NotCoveredAlpha 未被遮挡非描边部分透明度

  - | ![024-24](E:\Typora图片放置目录\024-24.jpg) | ![024-25](E:\Typora图片放置目录\024-25.jpg) |
    | ------------------------------------------- | ------------------------------------------- |
    | NotCoveredAlpha = 0                         | NotCoveredAlpha = 1                         |

  - NotCoveredEdgeAlpha 未被遮挡描边透明度(这里设置为0，就不会显示描边了)

  - | ![024-26](E:\Typora图片放置目录\024-26.jpg) | ![024-27](E:\Typora图片放置目录\024-27.png) |
    | ------------------------------------------- | ------------------------------------------- |
    | NotCoveredEdgeAlpha = 0                     | NotCoveredEdgeAlpha = 1                     |

**测试Demo：**

 [chooseItemsTest .rar](E:\Documents\Desktop\教程图片\rn图床\024\chooseItemsTest .rar) 